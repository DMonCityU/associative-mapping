...
def create_query_from_mention(mention, node, label, field, endpoints, window, node_labels_and_keys, similarity_threshold):
    """
...
def fetch_nodes(tx, node_labels_and_keys, override_query=None):
    ...
    if override_query:
        ...
        logger.critical(f"Default query overridden! If you don't know what you're doing, quit now!")
    ...
    for field in node_labels_and_keys[node_label]['fields']:
        ...
        if mentions_key in node:
            for mention in node[mentions_key]:
                ...
                if rdf_query:
                    if not dry_run:
                        run_query(session, rdf_query)
                        logger.debug(f"Running query")
                    rdf_queries += 1

                if merge_query:
                    if not dry_run:        
                        run_query(session, merge_query)
                        logger.debug(f"Running query")
                    merge_queries += 1
        else: 
...
def run_query(session, query, parameters=None):
    """
    Execute a given Cypher query using the provided Neo4j session.
